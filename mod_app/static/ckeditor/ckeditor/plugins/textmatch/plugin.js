...
(function () {
  ...
  CKEDITOR.plugins.textMatch.match = function (range, callback) {
    ...
    return {
      range: CKEDITOR.plugins.textMatch.getRangeInText(
        range,
        result.start,
        result.end + fillingSequenceOffset
      ),
  ...
  CKEDITOR.plugins.textMatch.getTextAndOffset = function (range) {
    ...
    if (startContainerIsText) {
      ...
      elementIndex =
        range.startOffset - (textNodes[0] ? textNodes[0].getIndex() : 0);
    }
  ...
  function findElementAtOffset(elements, offset) {
    ...
    for (var i = 0; i < max; i += 1) {
      ...
      if (
        ...
        currentOffset + current.getText().length >= offset
      ) {
        return {
          element: current,
